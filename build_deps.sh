...
clone https://github.com/redis/
hiredis.git hiredis 
--branch v1.0.2 --depth=1	echo 
"Hiredis downloaded" fi cd hiredis
    LIBRARY_PATH=lib CC=gcc 
    CXX=g++ make
    PREFIX="$(pwd)/
    /
    /build/install"
    USE_SSL=1
    static -j 4    LIBRARY_PATH=lib 
    CC=gcc CXX=g++
    make PREFIX="$(pwd)/
    /
    /build/install" 
...
if ls ../build/install/lib/libredis++.a 1>/dev/null 2>&1; then
 ...
 git clone https://github.com/sewenew/redis-plus-plus.git redis-plus-plus --branch 1.3.2 --depth=1	    echo "Redis-plus-plus downloaded"
    fi
    cd redis-plus-plus
    mkdir compile
    cd compile
    ...
    CC=gcc CXX=g++ make install    cd 
/echo "Finished installing Redis-plus-plus"fi
cd 
/
